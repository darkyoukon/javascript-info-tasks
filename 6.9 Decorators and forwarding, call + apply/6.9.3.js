"use strict";
console.log("The third task: Debounce decorator");
// The result of debounce(f, ms) decorator is a wrapper that suspends calls
// to f until there's ms of inactivity (no calls, “cooldown period”),
...
// transfers the latest call information to “the boss” (calls the actual f).
...
// In a web-browser, we can setup an event handler – a function that’s called
// on every change of an input field. Normally, an event handler is called
// very often, for every typed key. But if we debounce it by 1000ms,
...
// so its content is processed after 1000ms from the last input.
...
// It waits the given time after the last call,
// and then runs its function, that can process the result.
